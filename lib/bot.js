...
bot.on('speak', function(data){
  ...
  // Commands for all users
   ...
   if ((text.match(/^\/rockout$/)) || (text.match(/^\/ro$/))) {
      botRock++;
      if (botRock >= 3) {
        bot.vote('up');
      }
    ...
    if(res){
      ...
      if(result){
        ...
        switch(command){
          // general commands
          case 'afk':
            bot.speak('There is a 15 min Away From Keyboard limit for DJs. Please be sure to participate in the room (vote or chat) or you will be removed from the decks.');
            break;
          case 'avatar':
